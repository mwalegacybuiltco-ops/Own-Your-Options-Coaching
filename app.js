...
function coachReply(text) {
  ...
  const growth = growthInsights();
  const coach = selectedCoach();
  const opener = `${coach.name} here.`;
  if (!state.premium && /(business|income|sales|lwa|premium|nlp|belief|launch)/.test(lower)) {
    return `${opener} I remember your current focus is ${growth.focus.toLowerCase()}. Business is one part of your whole life, so start by asking: what do I need my work to support in my peace, family, health, freedom, and purpose? Then choose one grounded next step.`;
  }
  if (/(stuck|fear|scared|overwhelm|anxious)/.test(lower)) {
    return `${opener} I am noticing the pattern: ${growth.pattern.toLowerCase()}. Pause and separate the feeling from the option. What part of your life is asking for care right now, and what is one tiny step your future self can take without forcing?`;
  }
  if (/(goal|action|today|next)/.test(lower)) {
    return `${opener} Based on your ${growth.stage.toLowerCase()} stage, choose one action small enough to finish today and meaningful enough to count as evidence in your life. Write it as: I will do X by Y because I am becoming Z.`;
  }
  if (/(relationship|family|marriage|partner|friend|kids|children|home|love)/.test(lower)) {
    return `${opener} Let’s bring this into your relationships and home life. What option protects your peace and honors connection? Choose one honest conversation, boundary, or loving action that future you would respect.`;
  }
  if (/(health|body|wellbeing|well-being|energy|sleep|routine|stress)/.test(lower)) {
    return `${opener} Your body is part of the compass too. What is the kindest next signal you can give yourself today: rest, movement, water, food, space, or a simpler expectation?`;
  }
  if (/(business|income|lwa|sales|client)/.test(lower)) {
    return `${opener} Your OYO memory says your focus is ${growth.focus.toLowerCase()}. Let’s keep business inside your bigger life vision: clarify the life you want your work to support, then choose one aligned action that creates income without abandoning yourself.`;
  }
  return `${opener} I hear you. I am holding your future self statement and your current growth stage: ${growth.stage}. Bring it back to your whole life: "${state.futureSelf}" What option would your future self own in the next 20 minutes?`;
}
